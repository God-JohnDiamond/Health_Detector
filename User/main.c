/***************************************************
**Attribute:	辽宁科技大学毕业设计
**Production:	便携式健康检测仪的设计
**Author:		王柏东
**Port Connet：	OLED_I2C_SCL-->PB10
				OLED_I2C_SDA-->PB11
				MPU6050_I2C_SCL-->PB6
				MPU6050_I2C_SDA->PB7
				PULSERSENSOR_ADC-->PA1
***************************************************/
...
{  
    ...
    i2cInit();      //MPU6050的IIC总线的初始化    

	ADC1_Init();  			// 初始化ADC1用DMA方式 
	TIM2_Int_Init(20,7199); //2ms心率传感器任务调度
...
//		unsigned int runningTotal = 0;                  // clear the runningTotal variable    
		...
		while(dmp_get_pedometer_step_count(counter) == -1);			
		//串口打印步数
		PrintChar("step:");
		temp = *counter;
		...
		OLED_ShowCN(32,0,2);
		OLED_ShowCN(48,0,3);
		SHOW_SHUZI(64,0,temp/10000);
		SHOW_SHUZI(72,0,temp%10000/1000);
		SHOW_SHUZI(80,0,temp%1000/100);
		SHOW_SHUZI(88,0,temp%100/10);
		SHOW_SHUZI(96,0,temp%10);
		OLED_ShowCN(110,0,1);		
		
		//心率值串口打印
		PrintChar("BPM:");
		temp1 = (int)(BPM);		
		UsartSend(show_tab[temp1%1000/100]);
		UsartSend(show_tab[temp1%100/10]);
		UsartSend(show_tab[temp1%10]);
		PrintChar("\n");
		//OLED屏幕打印心率值
		OLED_ShowCN(0,2,4);
		OLED_ShowCN(16,2,5);
		OLED_ShowCN(32,2,6);
		OLED_ShowCN(48,2,7);
		SHOW_SHUZI(64,2,temp1%1000/100);
		SHOW_SHUZI(72,2,temp1%100/10);
		SHOW_SHUZI(80,2,temp1%10);
		OLED_Showchar(102, 2,'/');
		OLED_ShowCN(110,2,8);
    }
}
...
void TIM2_IRQHandler(void)
{
	...
	{		
		...
		Signal = ADC_ConvertedValue;              // read the Pulse Sensor 
